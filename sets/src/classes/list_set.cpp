...
template <> std::string toString<ListSet>(const ListSet &set) {
  std::string result;
  for (char c : UNIVERSE) {
    for (ListSet::ListNode *current = set.head; current != nullptr;
...
ListSet::ListSet(const ListSet &other) { copyFrom(other); }
...
ListSet::ListSet(ListSet &&other) noexcept
...
void ListSet::copyFrom(const ListSet &other) {
  ListNode **tail = &head;
  for (ListNode *current = other.head; current != nullptr;
       current = current->next) {
