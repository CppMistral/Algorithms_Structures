...
void print(const TestCase &test, const std::string &arrayResult,
           const std::string &listResult, const std::string &bitArrayResult,
           const std::string &machineWordResult, bool passed) {
  std::cout << "====================================================\n"
            << "Входные данные:\n"
            << "  A = " << formatSet(test.a) << '\n'
            << "  B = " << formatSet(test.b) << '\n'
...
bool run(const TestCase &test, bool verbose = false) {
  ...
  const bool passed = !test.expected.has_value() ||
                      (arrayResult == test.expected.value() &&
                       listResult == test.expected.value() &&
                       bitArrayResult == test.expected.value() &&
                       machineWordResult == test.expected.value());

  if (verbose || !passed) {
    print(test, arrayResult, listResult, bitArrayResult, machineWordResult,
          passed);
  }
...
void runBenchmarks() {
  ...
  const std::chrono::nanoseconds arrayTime =
      benchmarkFormula<ArraySet>(tests[0]);
  const std::chrono::nanoseconds listTime = benchmarkFormula<ListSet>(tests[0]);
  const std::chrono::nanoseconds bitArrayTime =
      benchmarkFormula<BitArraySet>(tests[0]);
  const std::chrono::nanoseconds machineWordTime =
      benchmarkFormula<MachineWordSet>(tests[0]);
